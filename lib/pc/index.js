...
module.exports = (data, staticData) => {
  ...
  return {
    loadDimensionCodec (codec) {
      ...
      require('fs').writeFileSync('./dimension-codec.json', JSON.stringify(dimensionCodec, null, 2))
      ...
      // Undocumented, just reading here for writing test
      data._dimensionData = codec.value['minecraft:dimension_type']
      // if (!dimensionData) {
      ...
      // for (const { name, id, element } of dimensionData) {
      //   data.dimensions[id] = element
      //   data.dimensionsByName[name] = element
      //   allDimensions.push(element)
      // }
      // data.dimensionsArray = allDimensions
